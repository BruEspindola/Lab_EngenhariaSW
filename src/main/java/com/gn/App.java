...
public class App extends Application {
    ...
    public synchronized void init(){
        ...
        load("controllerFXML", "relatorioController");
...
//        decorator.fullBody();
    ...
    public  void start(Stage primary) {
        ...
        decorator.getStage().getIcons().add(new Image("/logo2.png"));
        decorator.show();
    ...
    private void load(String module, String name){
        try {
            ViewManager.getInstance().put(
                    name,
                    FXMLLoader.load(getClass().getResource("/module/" + module + "/" + name + ".fxml"))
            );
            preloaderNotify();
        } catch (IOException e) {
            e.printStackTrace();
